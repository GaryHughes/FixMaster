...
suite('OrderBook Test Suite', () => {
    ...
    test('test unknown execution report', () => {
      const book = new OrderBook();
      const message = parseMessage("8=FIX.4.4\u00019=164\u000135=8\u000149=ACCEPTOR\u000156=INITIATOR\u000134=232\u000152=20190929-04:51:06.981\u000139=0\u000111=51\u000137=INITIATOR-ACCEPTOR-51\u000117=2\u0001150=0\u0001151=10000\u000155=WTF\u000154=1\u000138=10000\u000132=0\u000131=0\u000114=0\u00016=0\u000140=1\u000110=115\u0001");
      if (!message) {
    ...
    test('test order cancel replace request for unknown order', () => {
      const book = new OrderBook();
      const message = parseMessage("8=FIX.4.4\u00019=178\u000135=G\u000149=INITIATOR\u000156=ACCEPTOR\u000134=2536\u000152=20191117-01:01:47.010\u000137=INITIATOR-ACCEPTOR-56\u000141=56\u000111=57\u000170=55\u0001100=AUTO\u000155=WTF\u000154=1\u000160=20191117-01:01:38.158\u000138=100000\u000140=2\u000144=11\u000159=0\u000110=035\u0001");
      if (!message) {
        assert.fail("message failed to parse");
        return;
      }
    ...
    test('test order cancel request for unknown order', () => {
      const book = new OrderBook();
      const message = parseMessage("8=FIX.4.4\u00019=147\u000135=F\u000149=INITIATOR\u000156=ACCEPTOR\u000134=2544\u000152=20191117-01:09:11.302\u000141=58\u000137=INITIATOR-ACCEPTOR-58\u000111=59\u000155=WTF\u000154=1\u000160=20191117-01:09:09.139\u000138=100000\u000110=092\u0001");
      if (!message) {
        assert.fail("message failed to parse");
        return;
      }
    ...
    test('test order cancel reject for unknown order', () => {
      const book = new OrderBook();
      const message = parseMessage("8=FIX.4.49=127\u000135=949=ACCEPTOR\u000156=INITIATOR\u000134=511\u000152=20191117-01:11:06.578\u000137=INITIATOR-ACCEPTOR-58\u000139=8\u000141=58434=1\u000111=59\u000158=Unknown order\u000110=208\u0001");
      if (!message) {
        assert.fail("message failed to parse");
        return;
      }
      ...
      for (const text of messages) {
        ...
        switch (index) {
          ...
          case 1:
            {
            assert.equal(book.size, 1);
            const order = book.orders.values().next().value as Order;
      ...
      for (const text of messages) {
        ...
        switch (index) {
          ...
          case 4:
            ...
            const orders = book.orders.values();
            const original = orders.next().value as Order;
            const replacement = orders.next().value as Order;
